...
describe("teste de signUp", () => {
  ...
  test("Sucesso ao cadastrar o usuário", async () => {
    expect.assertions(1);
    try {
      const acessToken = await userBussinesMock.signup(
        "magdiel",
        "magdiel@gmail.com",
        "senha123",
        "NORMAL"
...
describe("teste de login", () => {
  ...
  test("Sucesso ao logar o usuario", async () => {
    expect.assertions(1);
    try {
      const acessToken = await userBussinesMock.login(
        "astrodev@gmail.com",
        "astrodev123"
      );
      expect(acessToken).toEqual({ accessToken: "token_mock" });
...
describe("Pegar usuário pelo id", () => {
  test("Usuário não existente", async() => {
      expect.assertions(2)
      try{
          await userBussinesMock.getUserById("aaa")
  ...
  test("Resposta de sucesso", async() => {
      expect.assertions(1)
      try {
          const getUserById = jest.fn((id: string)=> userBussinesMock.getUserById(id)) 
      
          const result = await getUserById("id_mock_1")

          expect(getUserById).toBe("id_mock_1")
